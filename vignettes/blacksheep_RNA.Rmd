...
deva_out = deva(blacksheep_SE,
    ...
    fraction_samples_cutoff = 0.3, fdrcutoffvalue = 0.1, write_out = FALSE)
```
...
### Tabulate Outliers
For each group, run through and output the number of outliers and nonoutliers 
per group. The output will be a list each named for the comparison inputted. 
For each comparison, there will be two items within it - namely the tabulated 
table, and then also the list of samples that went into that group.
